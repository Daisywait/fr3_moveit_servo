...
POSE_TRACKING_SETTINGS_YAML = "config/pose_tracking_settings.yaml"
...
def _pose_tracking_settings():
    pose_tracking_yaml = load_yaml("moveit_servo", POSE_TRACKING_SETTINGS_YAML)
    return {"moveit_servo": pose_tracking_yaml or {}}


def _merge_servo_params(*servo_param_blocks):
    merged = {}
    for params in servo_param_blocks:
        if not params:
            continue
        if "moveit_servo" in params and isinstance(params["moveit_servo"], dict):
            merged.update(params["moveit_servo"])
        elif isinstance(params, dict):
            merged.update(params)
    return {"moveit_servo": merged} if merged else {}


def _pose_tracking_node(
    robot_description,
...
def generate_launch_description():
    ...
    pose_tracking_node = _pose_tracking_node(
        robot_description,
        robot_description_semantic,
        robot_description_kinematics,
